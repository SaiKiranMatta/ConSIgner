...
def getBoxPlotDef(start, dimensions, color):
    ...
    x = [start["x"], start["x"], start["x"] + dimensions["width"], start["x"] + dimensions["width"],
         start["x"], start["x"], start["x"] + dimensions["width"], start["x"] + dimensions["width"]]
    y = [start["y"], start["y"] + dimensions["height"], start["y"] + dimensions["height"], start["y"],
         start["y"], start["y"] + dimensions["height"], start["y"] + dimensions["height"], start["y"]]
    z = [start["z"], start["z"], start["z"], start["z"], start["z"] + dimensions["length"], start["z"] +
         dimensions["length"], start["z"] + dimensions["length"], start["z"] + dimensions["length"]]
...
def saveContainerDataToFile(containerData):
    timestr = time.strftime("%Y%m%d-%H%M%S")
    filename = "files/Res-{0}.json".format(timestr)  # Add .json extension
    with open(filename, "w") as json_file:
        # Render without showing
        fig = renderContainerData(containerData, renderer="none")
...
def renderSampleContainer():
    ...
    MIN_EDGE_SIZE = 20
    MAX_EDGE_SIZE = 60
    boxes, boxes_cbm = simulateBoxData(
        NUMBER_OF_BOXES, MIN_EDGE_SIZE, MAX_EDGE_SIZE, 10, True)
...
def simulateBoxData(count, min_edge_size, max_edge_size, space_scaling_factor, save_data):
    """Create count number of boxes
        ...
        """
    boxes = np.random.random_integers(
        low=min_edge_size, high=max_edge_size, size=(count, 3))
    boxesDictArray = []
